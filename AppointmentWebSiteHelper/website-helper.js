...
let websiteHelper = (function () {
    ...
    websiteHelper.prototype.copyWebSiteAssets = function (resourceProperties, cb) {
        ...
        var apiGatewayInvokeURL = resourceProperties.apiGatewayInvokeURL;
        ...
        fs.readFile(_downloadLocation, 'utf8', function (err, data) {
            ...
            if (!_manifest) {
                ...
                uploadToS3(_manifest.files, 0, destS3Bucket, destS3KeyPrefix, region, apiGatewayInvokeURL, connectInstanceName,
                    function (err, result) {
                        if (err) {
                            return cb(err, null);
                        }
    ...
    function uploadToS3(filelist, index, destS3Bucket, destS3KeyPrefix, region, apiGatewayInvokeURL, connectInstanceName, cb) {
        if (filelist.length > index) {
            ...
            if (filelist[index].endsWith('.htm') || filelist[index].endsWith('.html')) {
                ...
                response = response.replace(/API_GATEWAY_INVOKE_URL/g, apiGatewayInvokeURL);
            ...
            s3.putObject(params2, function (err, data) {
                ...
                let _next = index + 1;
                uploadToS3(filelist, _next, destS3Bucket, destS3KeyPrefix, region, apiGatewayInvokeURL, connectInstanceName, function (err, resp) {
                    if (err) {
                        return cb(err, null);
                    }
