...
let websiteHelper = (function () {
    ...
    websiteHelper.prototype.copyWebSiteAssets = function (resourceProperties, cb) {
        ...
        fs.readFile(_downloadLocation, 'utf8', function (err, data) {
            ...
            if (!_manifest) {
                ...
                uploadToS3(_manifest.files, 0, destS3Bucket, destS3KeyPrefix, region, apiGatewayInvokeURL,
                    function (err, result) {
                        if (err) {
                            return cb(err, null);
                        }
    ...
    function uploadToS3(filelist, index, destS3Bucket, destS3KeyPrefix, region, apiGatewayInvokeURL, cb) {
        if (filelist.length > index) {
            ...
            s3.putObject(params2, function (err, data) {
                ...
                let _next = index + 1;
                uploadToS3(filelist, _next, destS3Bucket, destS3KeyPrefix, region, apiGatewayInvokeURL, function (err, resp) {
                    if (err) {
                        return cb(err, null);
                    }
